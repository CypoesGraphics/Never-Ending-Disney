...
function startMonitoringForSelectors(selectors, numTries) {
  ...
  const monitor = new MutationObserver(_ => {
    ...
    for (const elem of elems) {
      ...
      console.log("DEN - Tooltip " + !tooltip);
      if (ariaLabel === "INTRO ÜBERSPRINGEN" || ariaLabel.includes("NÄCHSTE FOLGE IN") || elem.classList.contains("skip__button")) {
        console.log("DEN - SKIP " + elem.classList)
        elem.click();
        ...
        function doGetPlayButton() {
          let play = document.querySelectorAll("play-pause-icon");
          let play_label = elem.getAttribute("aria-label");
          if (play_label === "Abspielen") {
            doClick(play)
          }
        ...
        console.log("DEN - NO ACTION FOUND FOR " + elem.classList);
        elem.click();
        elem.dispatchEvent(new PointerEvent('click'));
      }
    }
  ...
  let reactEntry = document.getElementById("webAppRoot");
  if (reactEntry) {
